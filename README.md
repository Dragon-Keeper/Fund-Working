...
### 0. 基金基本信息及申购状态管理系统 (1.Fund_Purchase_Status_Manager.py)
...
2. 运行：`python 1.Fund_Purchase_Status_Manager.py`
3. 菜单操作：
   - 选择1：下载基金基本信息（多线程）
   - 选择2：下载基金申购状态数据（多线程）
...
### 1. 通达信基金数据转HDF5工具 (2.TDX_To_HDF5.py)
...
#### 使用方法
直接运行程序：`python 2.TDX_To_HDF5.py`，然后根据提示输入相关参数：
...
### 2. 基金HDF5数据读取与分析系统 (2.Read_HDF5_Data.py)
...
#### 使用方法
直接运行程序：`python 2.Read_HDF5_Data.py`，然后在主菜单中选择所需功能：
...
### 3. 基金数据爬取与HDF5存储系统 (3.Fetch_Fund_Data.py)
