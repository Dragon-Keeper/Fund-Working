...
def query_currency_fund():
    ...
    if fund_data:
        ...
        print(f"最新万份收益: {fund_data['latest_yield']}")
        print(f"最新7日年化%: {fund_data['latest_annual']}%")
        print(f"成立日期: {fund_data['establish_date']}")
        print(f"基金经理: {fund_data['manager']}")
        print(f"手续费: {fund_data['fee']}")
        print(f"数据更新日期: {fund_data['update_date']}")
