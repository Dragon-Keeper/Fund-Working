...
class QuantAnalyzer:
    ...
    def __init__(self, hdf5_path=None, start_date_str=None):
        ...
        if hdf5_path:
            ...
            # 默认路径
            self.hdf5_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "All_Fund_Data.h5")
    ...
    def _clean_data(self, df):
        ...
        # 保留在正常范围内的数据
        df_clean = df_clean[(df_clean['daily_return'] >= lower_bound) & 
    ...
    def _calculate_sortino_ratio(self, df):
        ...
        sortino_ratio = (daily_return_mean * self.trading_days_per_year - self.risk_free_rate) / \
    ...
    def _perform_statistical_significance_tests(self, df):
        ...
        confidence_interval = stats.t.interval(0.95, len(daily_returns)-1, loc=mean_return, scale=std_error)
        
        # 转换为年化数据
        annualized_ci_lower = (confidence_interval[0] * self.trading_days_per_year) * 100
        annualized_ci_upper = (confidence_interval[1] * self.trading_days_per_year) * 100
        
        results['年化收益率95%置信区间'] = f"[{annualized_ci_lower:.2f}%, {annualized_ci_upper:.2f}%]"
        ...
        if len(daily_returns) <= 5000:  # Shapiro-Wilk检验对大样本敏感
            ...
            results['数据正态性'] = '符合正态分布' if sw_p_value > 0.05 else '不符合正态分布'
        ...
        try:
            with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
                ...
                if '年化收益率' in df.columns:
                    ...
                    worksheet.conditional_format(1, annual_return_col, len(df), annual_return_col, {
                        'type': 'cell',
                        'criteria': '<',
                        'value': 0,
                ...
                if '夏普率' in df.columns:
                    ...
                    worksheet.conditional_format(1, sharpe_col, len(df), sharpe_col, {
                        'type': 'cell',
                        'criteria': '>',
                        'value': 1,
                ...
                if '最大回撤率' in df.columns:
                    ...
                    worksheet.conditional_format(1, max_drawdown_col, len(df), max_drawdown_col, {
                        'type': 'cell',
                        'criteria': '>',
                        'value': 20,
                        'format': writer.book.add_format({'bg_color': '#FFC7CE', 'font_color': '#9C0006'})
...
def handle_all_funds_quant_analysis(analyzer):
    """处理所有基金的量化分析"""
    # 获取用户输入的时间范围
    start_date_str = input("请输入起始日期（YYYYMMDD格式，不输入则使用默认值）: ").strip()
    ...
    print("\n正在准备量化分析...")
    print(f"时间范围: {'从' + start_date_str + '开始' if start_date_str else '使用默认时间范围'} 至 {datetime.now().strftime('%Y-%m-%d')}")
